...
public final class L2EnchantSkillLearn {
    ...
    public void addEnchantDetail(EnchantSkillDetail esd) {
        ...
        if (enchantType < 0) {
            throw new IllegalArgumentException("Skill enchantments should have level higher than 100");
        } else {
            ...
            if (_enchantDetails[enchantType] == null) {
                _enchantDetails[enchantType] = new FastTable<>();
            }
            int index = L2EnchantSkillLearn.getEnchantIndex(esd.getLevel());
            while (index >= _enchantDetails[enchantType].size()) {
                _enchantDetails[enchantType].add(null);
            }
            _enchantDetails[enchantType].set(index, esd);
        }
    }
